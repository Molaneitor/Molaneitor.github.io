    ...
    <section id="experiencia" className="py-20 border-t border-[var(--color-border)]">
      <div className="section-container grid lg:grid-cols-5 gap-12">
        <div className="lg:col-span-3">
          <p className="section-eyebrow mb-3">{t(ui.experience.eyebrowWork)}</p>
          <h2 className="section-title mb-10">{t(ui.experience.titleWork)}</h2>

          <div className="space-y-8">
            {experience.map((job) => (
              <div key={job.company} className="relative pl-6 border-l border-[var(--color-border)]">
                <span
                  className="absolute -left-[5px] top-1.5 w-2.5 h-2.5 rounded-full"
                  style={{ backgroundColor: "var(--color-accent)" }}
                />
                <div className="flex flex-wrap items-baseline justify-between gap-2">
                  <h3 className="font-display font-semibold text-white text-lg">{t(job.role)}</h3>
                ...
                <p className="text-xs text-[var(--color-text-muted)] mt-0.5">{t(job.period)}</p>
                <ul className="mt-3 space-y-1.5 text-sm text-[var(--color-text-muted)]">
                  {job.bullets.map((b) => (
                    <li key={t(b)} className="flex gap-2">
        ...
        <div className="lg:col-span-2">
          <p className="section-eyebrow mb-3">{t(ui.experience.eyebrowEdu)}</p>
          <h2 className="section-title mb-10">{t(ui.experience.titleEdu)}</h2>
          <div className="space-y-6">
            {education.map((ed) => (
              <div key={ed.institution} className="card p-5">
                <h3 className="font-display font-semibold text-white">{t(ed.degree)}</h3>
